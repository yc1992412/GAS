...
## load data
xi_in_load = torch.load("./GAS_two/data/xi_in.pt").cpu()
xi_bound_all = torch.load(f'./GAS_two/data/xi_bound_add_all_{epoch}.pt')
xi_add_all = torch.load(f'./GAS_two/data/xi_add_all_{epoch}.pt')
xi_bound_add_all = torch.load(f'./GAS_two/data/xi_bound_add_all_{epoch}.pt')
...
u_exact_re = u_exact.reshape((opt.n_test, opt.n_test))
my_pinns = torch.load(f'./GAS_two/saved_model/my_pinns_model_{epoch}_{epoch_P}')
...
for i in range(opt.n_test):
    ...
    u_forward = my_pinns(tx_i)
    u_x, u_y = my_pinns.grad(tx_i)
