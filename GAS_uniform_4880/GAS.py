...
parser.add_argument("--n_epochs_adaptive", type=int, default=1, help="adaptive训练的epochs总数")
parser.add_argument("--epoch_P", type=float, default=50000, help="PINNs训练时的epoch次数")
parser.add_argument("--n_sample_in", type=int, default=4880, help="内部点需要的总样本数")
parser.add_argument("--n_sample_bound", type=int, default=1220, help="每条边上需要的样本数,n_sample_bound*4=n_sample_in/4")

parser.add_argument("--complete_batches", type=int, default=1000, help="用来学习PINNs的总batch数")
parser.add_argument("--batch_size_in", type=int, default=400, help="内部点每个batch的大小")
parser.add_argument("--batch_size_bound", type=int, default=100, help="边界点每个batch的大小, 应小于n_sample_bound/ndim")

parser.add_argument("--num_max_index", type=int, default=1, help="对残差最大的前opt.num_max_index个点构造高斯分布")
parser.add_argument("--Gaussian_number", type=int, default=1, help="从每个点处构造的高斯分布中选取Gaussian_number个点加到总样本当中")
parser.add_argument("--add_addpative_uniform", type=int, default=0, help="添加的自适应点和均匀分布点的比值")
parser.add_argument("--bound_add_number", type=int, default=15, help="每条边上加入到边界样本点中的个数,应保持bound_add_number*4*4 = num_max_index*Gaussian_number*(add_addpative_uniform+1)")
parser.add_argument("--sigma_penalty", type=float, default=5e-3, help="生成高斯分布时,方差前的修正参数,值越小高斯分布峰值越大")
...
class PINNsLoss(nn.Module):
    ...
    def forward(self, model, xi_in, xi_bound):
        ...
        loss = torch.square(Ggradgrad_x1 + Ggradgrad_x2 + loss_f)
...
def plot_and_save(epoch, epoch_P):
    ...
    plot2 = axs[0, 1].imshow(u_test_re, cmap='jet', extent =[opt.domain[0][0], opt.domain[0][1], opt.domain[1][0], opt.domain[1][1]], origin ='lower',vmin=0, vmax=1)
...
scheduler_P = torch.optim.lr_scheduler.MultiStepLR(optimizer_P, milestones = [1000, 2000, 5000, 10000, 15000, 20000, 30000, 40000], gamma = 0.8)
...
lossitem_P_list = np.zeros([opt.n_epochs_adaptive, 1])
for epoch in range(opt.n_epochs_adaptive):
    ...
    for epoch_P in range(opt.epoch_P):
        ...
        scheduler_P.step()
    lossitem_P_list[epoch] = loss_epoch_P.mean()
    print('='*10)        
    ...
    ###验证误差
    with torch.no_grad():
        residual_in_validate = (pinns(xi_in_all[:,0:2]).cpu()- u(xi_in_all[:,0:2]).cpu()).numpy()
        residual_in_validate **=2
    ...
    samples_by_residual = xi_in_all[residual_index[:,0],0:2]
    ...
    samples_by_residual_grad_x, samples_by_residual_grad_y = pinns.grad(samples_by_residual)
    mu = samples_by_residual.detach().cpu().numpy()
    ...
    sigma = opt.sigma_penalty/np.sqrt(sigma)
