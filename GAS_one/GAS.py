...
## 18000步之前的不修改
scheduler_P = torch.optim.lr_scheduler.MultiStepLR(optimizer_P, milestones = [15000, 20500], gamma = 0.7)
...
l2_err = np.zeros([int(opt.epoch_P * opt.n_epochs_adaptive/100), 1])
append_i = 0
for epoch in range(opt.n_epochs_adaptive):
    ...
    for i_index in range(min(complete_batches, opt.complete_batches)):
        ...
        residual_in_validate = torch.square(residual_gradgradx + residual_gradgrady + f_source(xi_in[:,0:opt.input_dim]).reshape((-1,1)))
